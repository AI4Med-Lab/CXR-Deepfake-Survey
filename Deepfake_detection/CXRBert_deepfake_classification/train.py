...
def evaluate(model, loader, device, criterion):
    ...
    for input_ids, attention_mask, labels in loader:
        ...
        total_loss += criterion(logits, labels.to(device)).item() * input_ids.size(0)
...
def main():
    ...
    train_ds = CXRReportDataset(args["csv"], "train", tokenizer, max_length=args["max_text_length"])
    val_ds = CXRReportDataset(args["csv"], "val", tokenizer, max_length=args["max_text_length"])
    ...
    # ---- Stage 1: linear probe on frozen CXR-BERT ----
    ...
    # ---- Stage 2: unfreeze last N transformer layers, fine-tune end-to-end ----
    ...
    best_finetune_path = os.path.join(args["model_save_dir"], "best_model_finetune.pt")
    best_probehead_path = os.path.join(args["model_save_dir"], "best_model_probehead.pt")
    final_best_path = best_finetune_path if os.path.exists(best_finetune_path) else best_probehead_path
    model.load_state_dict(torch.load(final_best_path))
